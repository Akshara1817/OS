...
int main() {
    ...
    int readyQ[100]; // simple queue for process indices
    int front = 0, rear = 0;
    ...
    for (int i = 0; i < n; i++) {
        scanf("%d", &p[i].bt);
        rem_bt[i] = p[i].bt;
        p[i].rt = p[i].bt; // here rt is remaining time
    ...
    while (completed < n) {
        ...
        // if current process still has burst time left, push it back
        if (rem_bt[idx] > 0) {
            readyQ[rear++] = idx;
        }
    }
